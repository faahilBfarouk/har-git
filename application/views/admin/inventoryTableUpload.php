...
<body>
                    ...
                    <div class="col-lg-12">
                        <div class="ibox float-e-margins">
                            ...
                            <div class="ibox-content text-center">
                                <form method="post" id="uploadForm" class="uploadForm" action="<?= ADMINURL ?>PhpspreadsheetController/upload_inv_exell">
                                            ...
                                            <tbody>
                                                <?php foreach ($data as $key => $row): ?>
                                                    <tr>
                                                        ...
                                                        <td>
                                                            <input type="text"  name="model_name[]" id="modelName<?= $key + 1 ?>" class="model_name" value="<?= $row['model_name'] ?>">
                                                        </td>
                                                        <td>
                                                            <input type="text"  name="variant[]" id="variant_<?= $key + 1 ?>" class="variant" value="<?= $row['variant'] ?>">
                                                        </td>
                                                        <td>
                                                            <input type="text"  name="sheet_name[]" id="sheet_name_<?= $key + 1 ?>" class="sheet_name" value="Inv" readonly="readonly">
                                                        </td>
    ...
    <script>
        ...
        function validateOrderDb() {
            ...
            $.each($(".model_name"), function () {
                Forms.append('modelNameId[]', $(this).attr('id'));
            });
//            Forms.forEach((value, key) => {
//                console.log(key + " " + value)
//            });
            $.ajax({
                url: admin_urlss + 'PhpspreadsheetController/get_inv_rep',
                method: 'POST',
                dataType: 'json',
                cache: false,
                contentType: false,
                ...
                success: function (data) {
                    ...
                    $.each(data['invInInvTabledata'], function (key, val) {
                        if (val.response === 0) {
                            ...
                            toastr["error"]("Procuct code in Line Number " + errorId + " a1 ready exist")
                        }
                    ...
                    if (invInInvTable === data['invInInvTabledata'].length) {
                        ...
                        //$('#import').html('Loading.....');
                        $.ajax({
                            url: admin_urlss + 'PhpspreadsheetController/upload_inv_exell',
                            method: 'POST',
                            dataType: 'json',
                            cache: false,
                            contentType: false,
        ...
        function validateDuplicate(result) {
            ...
            $('.product_code').removeClass('text-danger')
            $('.product_code').css({"background-color": ""});
            arr = [];
