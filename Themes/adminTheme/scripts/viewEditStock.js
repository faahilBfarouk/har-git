$(document).ready(function () {
    ...
    loadDataTable('table', 'sales/listStock')
    $('#search').click(function () {
        let stockTable = $('.table').DataTable()
        stockTable.destroy();
        filterDataTable('table', 'sales/listStock')
    })
...
function editstock(id, table) {
    $.ajax({
        ...
        success: function (data) {
            console.log(data.stock[0]['status_id'])
            $('.selectbox').empty()
            ...
            $('#Stockstatus').append(statusOption).trigger('change');
...
let deleteStock = function () {
    let table = $('#stocktable').DataTable();
    $('#stocktable tbody').on('click', '.delete', function () {
        console.log($(this).attr('data-chaseisNo'))
        let stock_id = $(this).attr('data-id');
        let chasisNo = $(this).attr('data-chaseisNo');
        Swal.fire({
            title: 'Are you sure?',
            ...
            if (result.value) {
                $.ajax({
                    url: admin_url + 'sales/stockDelete',
                    method: 'post',
                    dataType: 'json',
                    data: {
                        chasisNo: chasisNo,
